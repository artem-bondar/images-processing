...
template<typename ValueType>
void check_number(string val_name, ValueType val, ValueType from, 
				  ValueType to = numeric_limits<ValueType>::max())
{
    if (val < from)
...
{
	Matrix<double> matrix(MATRIX_ROWS, MATRIX_COLS);
	string::size_type start = 0, end;
	string number, sep = (matrix.n_cols > 1) ? "," : ";";

	for (uint i = 0; i < matrix.n_rows * matrix.n_cols - 1; i++, sep = ((i + 1) % matrix.n_cols) ? "," : ";") {
		if ((end = kernel.find(sep, start)) == std::string::npos || end == start)
			throw string("incorrect kernel matrix formatting");
		number = kernel.substr(start, end - start);
		if (!check_value<double>(number))
			throw string("incorrect number in matrix was used");
		matrix(i / matrix.n_cols, i % matrix.n_cols) = stod(number);
		start = end + 1;
	}
	number = kernel.substr(start);
	if (!check_value<double>(number))
		throw string("incorrect number in matrix was used");
	matrix(matrix.n_cols - 1, matrix.n_cols - 1) = stod(number);

    return matrix;
}

void parse_args(char **argv, int argc, bool *isPostprocessing, string *postprocessingType, double *fraction,
				bool *isMirror, bool *isInterp, bool *isSubpixel, double *subScale)
{
...
int main(int argc, char **argv)
{
    try {
        ...
        if (action == "--sobel-x") {
			...
			check_number("scale", scale, 0.0); // Added
		...
		} else if (action == "--white-balance") {
			check_argc(argc, 4, 4);
			dst_image = white_balance(src_image);
			...
			src_image = add_border(src_image, radius);
			src_image = mirror_border(src_image, radius);
            if (action == "--median") {
			...
			dst_image = cut_border(dst_image, radius);
